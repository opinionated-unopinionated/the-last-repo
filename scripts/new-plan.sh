#!/usr/bin/env bash
# Plan generator for The Last Repo.
# Copies docs/plans/TEMPLATE.md into docs/plans/<scope>/<NNNN>-<slug>.md,
# picking the next zero-padded sequence number per scope and seeding the
# frontmatter (id, title, scope, status, created, updated) so the author
# only has to fill in the body. status is forced to `draft` per ADR
# 0007 so freshly generated plans never pre-empt an in-progress plan.
# owner, loop_driver, and loop_prompt are left at their template defaults.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TEMPLATE="$REPO_ROOT/docs/plans/TEMPLATE.md"
BASE_DIR="$REPO_ROOT/docs/plans"

usage() {
  cat <<'EOF'
Usage: scripts/new-plan.sh <scope>/<slug>
       NAME=<scope>/<slug> scripts/new-plan.sh

Creates docs/plans/<scope>/<NNNN>-<slug>.md from docs/plans/TEMPLATE.md.
...
Examples:
  scripts/new-plan.sh platform/login-throttling
  make new-plan NAME=billing/invoice-pdf
EOF
}
...
awk -v id="$ID" -v title="$TITLE" -v scope="$SCOPE" -v today="$TODAY" '
  ...
  in_fm && /^status:/ { print "status: draft"; next }
